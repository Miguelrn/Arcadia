...
export class CompanyResolver {
	...
	@UseMiddleware(isAuth)
	async companyById(@Arg('companyId', () => Number) companyId: number): Promise<Company> {
	...
	async updateCompany(
		...
		@Arg('catch_phrase', () => String) catchPhrase: string,
		...
		company.catch_phrase = catchPhrase;
		company.logo = logo;
		company.type = type;
		company.phone = phone;
	...
	@Mutation(() => Boolean)
	@UseMiddleware(isAuth)
	async disableCompany(@Arg('companyId', () => Number) companyId: number): Promise<Boolean> {
		const company = await Company.findOne({
			where: { id: companyId },
			relations: ['workers'],
		});
