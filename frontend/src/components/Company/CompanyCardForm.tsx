...
export default function CompanyCardForm(props: WorkerCardInput) {
	const { data, loading } = useCompanyByIdQuery({
		variables: { companyId: Number.parseInt(props.id) },
	});
			...
			<Modal
				...
				<Box sx={style}>
					...
					<Grid container spacing={2} sx={{ mt: 2 }} justifyContent={'space-evently'}>
						<Grid item>
							<TextField id='companyId' label='Company Name' value={companyName} disabled />
						</Grid>
						<Grid item>
							<TextField
								id='industryId'
						...
						<Grid item>
							<TextField id='typeId' label='Industry Type' value={type} onChange={e => setType(e.target.value)} />
						</Grid>
						<Grid item>
							<TextField id='logoId' label='Logo' value={logo} disabled />
						</Grid>
						<Grid item>
