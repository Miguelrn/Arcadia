...
export default function WorkerCardForm(props: WorkerCardProp) {
	const { data, loading } = useWorkerByIdQuery({
		variables: { workerId: Number.parseInt(props.id) },
	});
	...
	return (
		<div>
			<Modal open={props.open} onClose={() => props.handleClose(props.id, false)} aria-labelledby='open worker info'>
				<Box sx={style}>
					...
					<TabPanel value={value} index={0}>
						<Grid container spacing={2} justifyContent={'space-evently'}>
							<Grid item>
								<TextField id='usernameId' label='Username' value={username} disabled />
							</Grid>
							<Grid item>
								<TextField id='nameId' label='Name' value={name} onChange={e => setName(e.target.value)} />
							</Grid>
							<Grid item>
								<TextField id='surnameId' label='Surname' value={surname} onChange={e => setSurname(e.target.value)} />
							</Grid>
							<Grid item>
								<TextField id='emailId' label='Email' value={email} onChange={e => setEmail(e.target.value)} />
							</Grid>
							<Grid item>
								<TextField id='avatarId' label='Avatar' value={avatar} disabled />
							</Grid>
							<Grid item>
								<TextField id='genderId' label='Gender' value={gender} onChange={e => setGender(e.target.value)} />
							</Grid>
							<Grid item>
								<TextField id='phoneId' label='Phone' value={phone} onChange={e => setPhone(e.target.value)} />
							</Grid>
							<Grid item>
								<TextField
									id='birthdateId'
						...
						<Grid container direction='row' justifyContent={'space-between'} sx={{ mt: 1 }}>
							<Button variant='contained' color='error' onClick={() => deleteWorker()}>
								<DeleteIcon />
								Delete
							</Button>
							<Button variant='contained' color='primary' onClick={() => updateWorker()}>
								<UpdateIcon />
								Update
							</Button>
						</Grid>
